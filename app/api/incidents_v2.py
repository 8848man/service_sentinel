...
router = APIRouter(prefix="/projects/{project_id}", tags=["Incidents (Project-Scoped)"])
...
def get_incidents(
    ...
    service_ids = [s.id for s in db.query(Service.id).filter(Service.project_id == project_id).all()]
    ...
    # Filter incidents to only those belonging to project's services
    ...
    # Further filter to ensure all incidents belong to project
    project_incidents = [i for i in incidents if i.service_id in service_ids]
